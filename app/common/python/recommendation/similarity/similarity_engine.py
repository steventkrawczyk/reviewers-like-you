...
from app.common.python.recommendation.similarity.similarity_computation import SimilarityComputation
from app.common.python.recommendation.similarity.similarity_shard import SimilarityShard
...
class SimilarityEngine:
    ...
    def get_closest_neighbor(self, input_vector: List[float]) -> str:
        ...
        for shard_index, shard in enumerate(self.shards):
            nn_index_from_shard, distance_from_shard = shard.get_closest_neighbor(
                input_vector)
            distances_from_shards.append(distance_from_shard)
            author_index[shard_index] = shard.decode_match(nn_index_from_shard)
        nn_index = SimilarityComputation.driver_computation(
            distances_from_shards)
