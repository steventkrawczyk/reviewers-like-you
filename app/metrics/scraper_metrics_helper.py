...
class ScraperMetricsHelper(TimedMetricsHelper):
    ...
    def record_scraper_performance(self, number_of_entries: int, entries_attempted: int) -> None:
        if self.time_in_ns == 0:
            logging.error("You should not call record_scraper_performance without running timer or passing in time_in_ns")
        self.record_scraper_performance(number_of_entries, entries_attempted, self.time_in_ns)
