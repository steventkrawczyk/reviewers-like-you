...
void get_average_handler(const shared_ptr<Session> session) {
    session->close(OK, "Hello, World!", {{"Content-Length", "13"}, {"Connection", "close"}});
}

void post_closest_handler( const shared_ptr< Session > session ) {
    session->close(OK, "Hello, World!", {{"Content-Length", "13"}, {"Connection", "close"}});
}

int main(const int, const char**) {
    auto average_resource = make_shared<Resource>();
    average_resource->set_path("/average");
    average_resource->set_method_handler("GET", get_average_handler);

    auto closest_resource = make_shared<Resource>();
